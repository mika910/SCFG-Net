...
class MaxViT4Out_Small(nn.Module):
    def __init__(self, n_class=1, img_size=224, pretrain=True):
        ...
        if img_size == 224:
            self.backbone = maxvit_rmlp_small_rw_224_4out()  # [64, 128, 320, 512]
            if pretrain:
                print('Loading:', 'networks/merit_lib/pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
                state_dict = torch.load(
                    'networks/merit_lib/pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
                # print('Loading:', r'D:\gitcodeANDpapers\medical image segmentation\code\MSA-2Net-main\model_out\SFG110\msa2net_best_hd95_epoch_189.pth')
                # state_dict = torch.load(r"D:\gitcodeANDpapers\medical image segmentation\code\MSA-2Net-main\model_out\SFG110\msa2net_best_hd95_epoch_189.pth")
        elif (img_size == 256):
            self.backbone = maxxvit_rmlp_small_rw_256_4out()
            print('Loading:', './pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
            state_dict = torch.load('./pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
