...
class EfficientSelfAtten(nn.Module):
    def __init__(self, dim, head, reduction_ratio):
        ...
        if reduction_ratio > 1:
            # downscales the input feature
            # sr Spatial Reduction
            # reduction_ratio r: kernel and stride size
            # the spatial size of the input will be reduced by a factor of r in both height and width dimensions
            # [B, C, H / r, W / r]
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.head, C // self.head).permute(0, 2, 1, 3) #(B, self.head, N, C // self.head)

        if self.reduction_ratio > 1:
            p_x = x.clone().permute(0, 2, 1).reshape(B, C, H, W)
            sp_x = self.sr(p_x).reshape(B, C, -1).permute(0, 2, 1) #(B, N/r^2, C)
            x = self.norm(sp_x)
        # x 已降维
        kv = self.kv(x).reshape(B, -1, 2, self.head, C // self.head).permute(2, 0, 3, 1, 4) #(2, B, self.head, N/r^2, C // self.head)
        k, v = kv[0], kv[1] #(B, self.head, N/r^2, C // self.head)
        # (B, self.head, N, C // self.head) @ (B, self.head, C // self.head, N/r^2)
        attn = (q @ k.transpose(-2, -1)) * self.scale #  (B, self.head, N, N/r^2)
        attn_score = attn.softmax(dim=-1)# B * N*N * C-> B * N*N/r*r * C
...
class Scale_reduce(nn.Module):
    def __init__(self, dim, reduction_ratio):
        ...
        if len(self.reduction_ratio) == 4:
            self.sr0 = nn.Conv2d(dim, dim, reduction_ratio[3], reduction_ratio[3]) #1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
        if len(self.reduction_ratio) == 4:
            tem0 = x[:, :3136, :].reshape(B, 56, 56, C).permute(0, 3, 1, 2) # 56 * 56 = 3136
            tem1 = x[:, 3136:4704, :].reshape(B, 28, 28, C * 2).permute(0, 3, 1, 2)  # 28*28*2=4704-3136=1568
            tem2 = x[:, 4704:5684, :].reshape(B, 14, 14, C * 5).permute(0, 3, 1, 2)  # 14*14*5 = 5684-4704
            tem3 = x[:, 5684:6076, :]
...
# B0 : image_size 224;dims [32, 64, 160, 256];layers[2, 2, 2, 2]
class MiT(nn.Module):
    def __init__(self, image_size, dims, layers, token_mlp="mix_skip"):
        ...
        # patch_embed
        self.patch_embed1 = OverlapPatchEmbeddings(image_size, patch_sizes[0], strides[0], padding_sizes[0], 3, dims[0]) # 224+6-7/4 + 1 = 56
...
class SegFormer(nn.Module):
    def __init__(self, model_name: str = "B0", num_classes: int = 19, image_size: int = 224) -> None:
        ...
        self.backbone = MiT(image_size, dims, layers) # 224;[32, 64, 160, 256];[2, 2, 2, 2]
