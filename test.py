...
parser.add_argument("--test_save_dir", type=str, default="./model_out/test", help="saving prediction as nii!") #"./predictions"
...
parser.add_argument("--model_name", type=str, default="SCFG", help="model_name")
...
if __name__ == "__main__":
    ...
    net = SCFG_Net(num_classes=4).cuda()

    snapshot_path = "./model_out"
    snapshot = os.path.join(snapshot_path, 'best_model.pth')
    if not os.path.exists(snapshot):
        snapshot = snapshot.replace("best_model", "SCFG_Net_epoch_399" )
        
    net.load_state_dict(torch.load(snapshot))
    print("self trained Sina unet")

    log_folder = "./model_out/test"
    os.makedirs(log_folder, exist_ok=True)
    logging.basicConfig(
        filename=log_folder + "/" + "SCFG" + ".txt",
        level=logging.INFO,
